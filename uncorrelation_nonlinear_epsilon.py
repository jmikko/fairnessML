...
class Fair_SVM_eps(BaseEstimator):
    def __init__(self, kernel='rbf', C=1.0, sensible_feature=None, gamma=1.0, epsilon=0.0):
        self.kernel = kernel
        self.C = C
    ...
    def fit(self, X, y):
        ...
        if self.fairness:
            ...
            G = cvxopt.matrix(np.vstack([G, fairness_line, -fairness_line]))
            h = cvxopt.matrix(np.vstack((h, self.epsilon, self.epsilon)))
...
if __name__ == "__main__":
    ...
    algorithm = Fair_SVM_eps(sensible_feature=sensible_feature, epsilon=0.01)
